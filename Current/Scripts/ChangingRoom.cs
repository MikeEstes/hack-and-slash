...
//	private PlayerCharacter pc;
	...
	private void InstantiateHairMesh() {
		...
		if( PC.Instance.hairMount.transform.childCount > 0)
			for(int cnt = 0; cnt < PC.Instance.hairMount.transform.childCount; cnt++)
				Destroy(PC.Instance.hairMount.transform.GetChild(cnt).gameObject);
		
		GameObject w = Instantiate(ca.hairMesh[_hairMeshIndex], PC.Instance.hairMount.transform.position, Quaternion.identity) as GameObject;
		w.transform.parent = PC.Instance.hairMount.transform;
...
//		Resources.UnloadUnusedAssets();
	...
	private void InstantiateWeaponModel() {
		...
		GameObject w = Instantiate(ca.weaponMesh[_weaponIndex], PC.Instance.weaponMount.transform.position, Quaternion.identity) as GameObject;
